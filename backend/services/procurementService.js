...
export const createOrder = async (userId, order) => {
  ...
  const result = await pool.query(
    `INSERT INTO procurement_orders 
      (user_id, supplier, quantity, price_per_kg, transport_cost, delivery_date, total_cost, notes) 
     VALUES ($1, $2, $3, $4, $5, $6, $7, $8) 
     RETURNING *`,
    [userId, supplier, quantity, price_per_kg, transport_cost, delivery_date, total_cost, notes]
  );
...
export const updateOrder = async (userId, id, order) => {
  ...
  const result = await pool.query(
    `UPDATE procurement_orders SET 
     ...
     RETURNING *`,
    [supplier, quantity, price_per_kg, transport_cost, delivery_date, total_cost, notes, id, userId]
  );

...
export const getBatchesByUser = async (userId) => {
  const res = await pool.query(
    "SELECT * FROM production_batches WHERE user_id = $1 ORDER BY created_at DESC",
    [userId]
  )
  return res.rows
}
...
export const createBatch = async (userId, batchData) => {
  ...
  const res = await pool.query(
    `INSERT INTO production_batches 
     ...
     RETURNING *`,
    [userId, maizeQuantity, flourOutput, branOutput, waterUsage, electricityUsage, sacksUsed, employeeNotes]
  )
  return res.rows[0]
}
...
export const updateBatch = async (userId, batchId, batchData) => {
  ...
  const res = await pool.query(
    `UPDATE production_batches SET
     ...
     RETURNING *`,
    [maizeQuantity, flourOutput, branOutput, waterUsage, electricityUsage, sacksUsed, employeeNotes, batchId, userId]
  )
  return res.rows[0]
}

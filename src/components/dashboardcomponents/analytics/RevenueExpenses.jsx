...
const RevenueExpenses = ({ data, darkMode, formatNumber }) => {
        ...
        <div className={`p-3 rounded-lg shadow-lg ${darkMode ? "bg-gray-800 border border-gray-700" : "bg-white border border-gray-200"}`}>
          ...
          {payload.map((entry, index) => (
            <p key={index} className={`text-sm`} style={{ color: entry.color }}>
              {`${entry.name}: Tsh ${formatNumber ? formatNumber(entry.value) : entry.value.toLocaleString()} `}
            </p>
          ))}
        </div>
      )
  ...
  if (filteredData.length === 0) {
    return (
      <motion.div
        ...
        className={`p-6 rounded-xl shadow-lg mt-4 ${darkMode ? "bg-gray-800" : "bg-white"} border ${darkMode ? "border-gray-700" : "border-gray-200"}`}
      >
    ...
    >
      ...
      <ResponsiveContainer width="100%" height={400}>
        <BarChart data={paginatedData} margin={{ top: 5, right: 30, left: 20, bottom: 60 }}>
          ...
          <Legend />
          <Bar dataKey="revenue" fill="#10B981" name="Revenue (Tsh)" radius={[4, 4, 0, 0]} />
          <Bar dataKey="expenses" fill="#EF4444" name="Expenses (Tsh)" radius={[4, 4, 0, 0]} />
        </BarChart>
      </ResponsiveContainer>
      ...
      {filteredData.length > itemsPerPage && (
        <div className="flex justify-center items-center gap-4 mt-6">
          ...
          <span className={`text-sm font-medium ${darkMode ? "text-gray-300" : "text-gray-700"}`}>
            Page {page} of {totalPages} (Showing {((page - 1) * itemsPerPage) + 1}-{Math.min(page * itemsPerPage, filteredData.length)} of {filteredData.length} days)
          </span>
          <button 
